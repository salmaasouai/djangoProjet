...
def lire_offre_vocale(request, offre_id):
    ...
    texte_a_lire = f"Offre: {offre_instance.titleOffre}. Programme: {offre_instance.program}, Capacité: {offre_instance.capacity}, Prix: {offre_instance.price}."
...
def analyze_face(request, offre_id):
    """
    ...
    offre_instance = get_object_or_404(offre, id=offre_id)  # Récupération de l'offre associée
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not response_message["age"]:
                        ...
                        if age == 18:
                            filtered_offers = offre.objects.filter(price__lte=50)  # Exemple : offres à prix <= 50
                        elif age < 18:
                            filtered_offers = offre.objects.filter(capacity__gte=5)  # Exemple : offres avec une capacité >= 5
                        else:
                            filtered_offers = offre.objects.filter(price__gte=100)  # Exemple : offres plus chères
                else:
    ...
    return render(
        ...
        "face_analyzer.html",  # Modifiez le chemin si nécessaire
        {
            ...
            "offre": offre_instance,  # Passer l'offre renommée ici
            "error": error_message,
